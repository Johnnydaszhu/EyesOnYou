...
struct GlobalSearchBar: View {
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            .strokeBorder(
                                LinearGradient(
                                    colors: focused
                                        ? [EyesOnYouTheme.accentBlue.opacity(0.7), Color.white.opacity(0.15)]
    ...
    private func color(for kind: AppModel.SearchHitKind) -> Color {
        switch kind {
        case .app: return EyesOnYouTheme.accentBlue
        case .site: return EyesOnYouTheme.accentPurple
        case .rule: return EyesOnYouTheme.accentAmber
        case .group: return EyesOnYouTheme.gold
        }
    }
    ...
    var body: some View {
        ...
        Button {
            ...
            Image(systemName: on ? "star.fill" : "star")
                ...
                .foregroundStyle(on ? EyesOnYouTheme.gold : EyesOnYouTheme.textSecondary.opacity(0.7))
        }
        .buttonStyle(.plain)
