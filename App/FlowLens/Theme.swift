...
struct LiquidGlassWindowBackdrop: View {
    ...
    var body: some View {
        let isLight = colorScheme == .light
        let mono = FlowLensTheme.isMono
        ZStack {
            FlowLensTheme.background

            if isLight {
                Circle()
                    .fill(FlowLensTheme.brandGreen.opacity(mono ? 0.04 : 0.06))
                    .frame(width: 480, height: 480)
                    .blur(radius: 90)
                    .offset(x: -200, y: -160)
                Circle()
                    .fill((mono ? FlowLensTheme.routeProxy : FlowLensTheme.gold).opacity(mono ? 0.04 : 0.05))
                    .frame(width: 360, height: 360)
                    .blur(radius: 80)
                    .offset(x: 220, y: 200)
            } else {
                Circle()
                    .fill(FlowLensTheme.accentBlue.opacity(mono ? 0.06 : 0.16))
                    .frame(width: 420, height: 420)
                    .blur(radius: 80)
                    .offset(x: -220, y: -180)
                Circle()
                    .fill(FlowLensTheme.accentPurple.opacity(mono ? 0.05 : 0.12))
                    .frame(width: 380, height: 380)
                    .blur(radius: 90)
                    .offset(x: 260, y: 40)
                Circle()
                    .fill((mono ? FlowLensTheme.routeProxy : FlowLensTheme.gold).opacity(mono ? 0.07 : 0.08))
