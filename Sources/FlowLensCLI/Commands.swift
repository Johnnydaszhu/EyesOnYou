...
enum FavoritesStore {
    static func allKeys() -> [String] {
        if let arr = UserDefaults.standard.array(forKey: FlowLensPaths.favoritesDefaultsKey) as? [String] {
            return arr.sorted()
        }
        if let data = try? Data(contentsOf: FlowLensPaths.favoritesFile),
    ...
    static func set(_ keys: [String]) throws {
        ...
        UserDefaults.standard.set(unique, forKey: FlowLensPaths.favoritesDefaultsKey)
        try FlowLensPaths.ensureSupportDir()
        let data = try JSONEncoder().encode(unique)
        try data.write(to: FlowLensPaths.favoritesFile, options: .atomic)
    }
...
func cmdStatus(opts: GlobalOptions) throws -> ExitCode {
    ...
    let dbExists = FileManager.default.fileExists(atPath: FlowLensPaths.telemetryDB.path)
    let payload: [String: Any] = [
        "ok": true,
        "version": CLIRunner.version,
        "mode": dbExists ? "sqlite" : "demo",
        "paths": [
            "support": FlowLensPaths.supportDir.path,
            "telemetry_db": FlowLensPaths.telemetryDB.path,
            "favorites_file": FlowLensPaths.favoritesFile.path
        ],
    ...
    emit(payload, json: opts.json) {
        print("flowlens \(CLIRunner.version)")
        print("mode: \(dbExists ? "sqlite" : "demo")")
        print("support: \(FlowLensPaths.supportDir.path)")
...
func cmdPaths(opts: GlobalOptions) -> ExitCode {
    let payload: [String: Any] = [
        ...
        "telemetry_db": FlowLensPaths.telemetryDB.path,
        "favorites_file": FlowLensPaths.favoritesFile.path,
        "favorites_defaults_key": FlowLensPaths.favoritesDefaultsKey
    ]
    emit(payload, json: opts.json) {
        print(FlowLensPaths.supportDir.path)
        print(FlowLensPaths.telemetryDB.path)
        print(FlowLensPaths.favoritesFile.path)
    }
    return .ok
}
...
func cmdSearch(opts: GlobalOptions) throws -> ExitCode {
    ...
    guard !q.isEmpty else { throw CLIError.usage("search requires a query: flowlens search <query>") }
