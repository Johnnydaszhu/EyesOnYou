...
enum CLIRunner {
    ...
    private static func dispatch(command: String, opts: GlobalOptions) throws -> ExitCode {
        ...
        case "version", "--version", "-V":
            emit(["name": "flowlens", "version": version, "platform": "macOS"], json: opts.json) {
                print("flowlens \(version)")
            }
            return .ok
        case "status":
...
enum FlowLensPaths {
    static var supportDir: URL {
        ...
        return base.appendingPathComponent("FlowLens", isDirectory: true)
    }
    ...
    static let favoritesDefaultsKey = "flowlens.favoriteAppKeys"
...
func helpText(json: Bool) -> String {
    ...
    return """
    flowlens — FlowLens CLI for humans and coding agents (Codex, Claude Code, …)

    USAGE
      flowlens [--json] <command> [options]
    ...
    EXAMPLES (agent-friendly)
      ...
      flowlens --json traffic --app com.google.Chrome --period day
...
func agentManifest() -> [String: Any] {
    [
        ...
        "description": "Native macOS network observability CLI for FlowLens. Prefer --json for agent use.",
        "install": "swift run flowlens   # from repo root; or: swift build -c release && .build/release/flowlens",
        "global_flags": [
        ...
        "notes_for_agents": [
            ...
            "Favorites share the host app UserDefaults key flowlens.favoriteAppKeys when available.",
