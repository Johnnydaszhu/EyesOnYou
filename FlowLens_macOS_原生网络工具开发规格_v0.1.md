# FlowLens：macOS 原生网络可观测、防火墙与选择性代理开发规格
...
```mermaid
flowchart LR
    UI[FlowLens 主 App\nSwiftUI + AppKit] <-->|签名校验 XPC| SYX[FlowLens.systemextension]
...
├─ FlowLensApp                     macOS Application
├─ FlowLensNetworkExtension        Network Extension System Extension
│  ├─ FlowFilterDataProvider
│  └─ FlowTransparentProxyProvider
├─ FlowLensCore                    Swift Package / Framework
├─ FlowLensRuleEngine              Swift Package
├─ FlowLensStorage                 Swift Package
├─ FlowLensIPC                     Swift Package
├─ FlowLensProxyCore               Swift Package
├─ FlowLensTestSupport             Test helpers
├─ FlowLensCoreTests
├─ FlowLensRuleEngineTests
├─ FlowLensStorageTests
├─ FlowLensProxyCoreTests
└─ FlowLensIntegrationTests
```
...
| UI 文案 | 条件 | 置信度 |
|---|---|---:|
| 已通过 FlowLens 代理 | Transparent Proxy session 已建立 | 100 |
| 已直连（由 FlowLens 决定） | Proxy Provider 对该 flow 返回 direct | 100，限于本层决定 |
...
“FlowLens 决定 direct”只说明我们的 Transparent Proxy 未接管；如果系统级 VPN/代理在更下层继续处理，最终链路仍可能不是物理直连。UI 应显示两层：
...
```swift
@objc protocol FlowLensProviderControl {
...
```text
时间 | App | 目标 | 协议 | 上/下行 | 防火墙 | FlowLens 路由 | 系统网络层 | 状态
```
...
```text
Host App:          com.example.FlowLens
System Extension:  com.example.FlowLens.NetworkExtension
...
<string>FlowLens 使用系统网络扩展统计、控制并按规则路由应用连接。</string>
<key>NetworkExtension</key>
<dict>
  <key>NEMachServiceName</key>
  <string>$(FLOWLENS_MACH_SERVICE)</string>
